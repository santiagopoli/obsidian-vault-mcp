...
- GitHub OAuth state uses a ten-minute host-only secure cookie. Consent state is a separate one-time, ten-minute KV value consumed atomically by the approval form.
...
Cloudflare KV stores OAuth provider state and grants. The provider hashes token material and encrypts grant properties. D1 stores webhook delivery metadata, vault checkpoints, derived note events, and automation run status. The graph cache may temporarily contain derived private metadata—paths, titles, aliases, tags, and edges—keyed by repository and immutable tree revision. Raw note contents and raw webhook bodies are not persisted in KV, D1, or the graph cache.
